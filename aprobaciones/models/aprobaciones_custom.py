...
class AprobacionesCustom(models.Model):
    ...
    fecha_creacion=fields.Date(string="Fecha de creacion", default=fields.Date.today())
    ...
    aprobaciones_lines_ids = fields.One2many("aprobaciones.lines", "aprobacion_id", string="Detalles")

    category_ids =fields.Many2many('aprobaciones.category', string="Categoria")
    ...
    @api.constrains('aprobaciones_lines_ids')
    def _check_min_max_aprobaciones(self):
        for record in self:
            if len(record.aprobaciones_lines_ids) < 1:
                raise ValidationError("Debes de tener al menos una aprobacion")
